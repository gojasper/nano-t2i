    ...
    def __call__(self, batch: dict) -> bool:
        try:
            ...
            if not res and self.verbose:
                logging.info(
                    f"KeyFilter: {self.keys} not found in batch: {batch.keys()}"
                )
            return res
...
class FilterOnCondition(BaseFilter):
    ...
    def __call__(self, batch: dict) -> bool:
        if self.condition_key not in batch:
            if self.verbose:
                logging.info(
                    f"FilterOnCondition: {self.condition_key} not in batch. Keeping sample: {not self.strict}"
                )
            if self.strict:
                return False
            else:
            ...
            try:
                keep_sample = self.condition_fn(condition_key)
            except Exception as e:
                if self.verbose:
                    logging.error(
                        f"Error in FilterOnCondition: {e}. Condition key: {condition_key}, url: {batch['__url__']}"
                    )
                return False
            if self.verbose and not keep_sample:
                logging.info(
                    f"FilterOnCondition: {self.condition_key} not met for sample. Condition key value: {condition_key}"
                )
            return keep_sample
