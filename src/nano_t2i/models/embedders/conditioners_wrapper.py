...
class ConditionerWrapper(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        for conditioner in self.conditioners:
            batch_length = len(batch[conditioner.input_key])
            if conditioner.input_key in ucg_keys:
                force_zero_embedding = torch.ones(batch_length)
            elif conditioner.ucg_rate > 0 and not set_ucg_rate_zero:
                force_zero_embedding = torch.rand(batch_length) < conditioner.ucg_rate
            else:
                force_zero_embedding = torch.zeros(batch_length)
