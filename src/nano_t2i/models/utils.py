...
def truncate_prediction_and_target(
    latent_prediction: torch.Tensor,
    pixel_target: torch.Tensor,
    vae_downsampling_factor: int = None,
    pixel_loss_max_size: int = 512,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """Truncate with the same coordinates pixel_target and latent_prediction (up to vae_downsampling_factor if provided) according to pixel_loss_max_size.

    Args:
        latent_prediction (torch.Tensor of shape(batch_size,*))
            Prediction in latent space (or pixel space if vae_downsampling_factor = None).

        pixel_target (torch.Tensor of shape(batch_size,*))
            Target in pixel space.

        vae_downsampling_factor (int)
            VAE downsampling factor. Defaults to None

        pixel_loss_max_size (int)
            Maximum size of the truncated outputs in pixel space. Defaults to 512.

    Returns:
        latent_prediction (torch.Tensor of shape(batch_size,*))
            Truncated prediction in the same space as latent_prediction.

        pixel_target (torch.Tensor of shape(batch_size,*))
            Truncated target in pixel space.
    """
    if vae_downsampling_factor is not None:
        latent_crop = pixel_loss_max_size // vae_downsampling_factor
    else:
        latent_crop = pixel_loss_max_size

    pred_crop_h = max((latent_prediction.shape[2] - latent_crop), 0)
    pred_crop_w = max((latent_prediction.shape[3] - latent_crop), 0)

    target_crop_h = max((pixel_target.shape[2] - pixel_loss_max_size), 0)
    target_crop_w = max((pixel_target.shape[3] - pixel_loss_max_size), 0)

    # image random cropping
    if pred_crop_h == 0:
        pred_offset_h = 0
    else:
        pred_offset_h = torch.randint(0, pred_crop_h + 1, (1,)).item()
    if pred_crop_w == 0:
        pred_offset_w = 0
    else:
        pred_offset_w = torch.randint(0, pred_crop_w + 1, (1,)).item()
    if vae_downsampling_factor is not None:
        target_offset_h = pred_crop_h * vae_downsampling_factor
        target_offset_w = pred_offset_w * vae_downsampling_factor
    else:
        target_offset_h = pred_offset_h
        target_offset_w = pred_offset_w

    latent_prediction = latent_prediction[
        :,
        :,
        pred_offset_h : latent_prediction.shape[2] - (pred_crop_h - pred_offset_h),
        pred_offset_w : latent_prediction.shape[3] - (pred_crop_w - pred_offset_w),
    ]

    pixel_target = pixel_target[
        :,
        :,
        target_offset_h : pixel_target.shape[2] - (target_crop_h - target_offset_h),
        target_offset_w : pixel_target.shape[3] - (target_crop_w - target_offset_w),
    ]

    return latent_prediction, pixel_target
...
def kaiming_uniform_zeros_(m: torch.nn.Parameter):
    """Inplace Kaiming initialization to 0 of target parameters.

    :param m : the target parameters
    """
    return torch.nn.init.kaiming_uniform_(
        m, a=math.sqrt((6.0 / init_weight_scale**2) - 1)
    )


def init_bias_uniform_zeros(m: torch.nn.Parameter, weight: torch.nn.Parameter):
    """Inplace initialization to 0 of target 'bias' parameters.
    This is done regarding the value of reference parameters

    :param m : the target 'bias' parameters
    :param weight : the reference parameters
    """
